<?php
// processar_yoga.php
// Processa inserção ou atualização de aula de Yoga
...
if (isset($_GET['id'])) {
    ...
    if ($imagem) {
        $stmt = $mysqli->prepare("UPDATE yoga SET imagem=?, titulo=?, horarios=?, dia=?, valor=?, turma=?, descricao=?, atualizado_por=?, atualizado_em=? WHERE id=?");
        $stmt->bind_param("sssssdsssi", $imagem, $titulo, $horarios, $dia, $valor, $turma, $descricao, $atualizado_por, $atualizado_em, $id);
    } else {
        $stmt = $mysqli->prepare("UPDATE yoga SET titulo=?, horarios=?, dia=?, valor=?, turma=?, descricao=?, atualizado_por=?, atualizado_em=? WHERE id=?");
    ...
    // Novo cadastro
    $stmt = $mysqli->prepare("INSERT INTO yoga (imagem, titulo, horarios, dia, valor, turma, descricao, atualizado_por, atualizado_em) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssdsss", $imagem, $titulo, $horarios, $dia, $valor, $turma, $descricao, $atualizado_por, $atualizado_em);
    $stmt->execute();
    $stmt->close();
}

// Redirecionar para listar_yoga.php
