...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($acao === "inserir") {
        $stmt = $mysqli->prepare("INSERT INTO noticias (titulo, data, dias, horarios, mensalidade, descricao, cancelado) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssi", $titulo, $data, $dias, $horarios, $mensalidade, $descricao, $cancelado);
        $stmt->execute();
    } elseif ($acao === "editar" && $id > 0) {
        $stmt = $mysqli->prepare("UPDATE noticias SET titulo = ?, data = ?, dias = ?, horarios = ?, mensalidade = ?, descricao = ?, cancelado = ? WHERE id = ?");
