<?php
// admin/reflexao/reflexao_edit.php
include_once("../includes/_header.php");
include_once("../includes/conexao.php");

if (!isset($_GET["id"])) {
    header("Location: listar_reflexao.php");
    exit;
}

$id = intval($_GET["id"]);
$stmt = $mysqli->prepare("SELECT * FROM reflexao WHERE id = ?");
...
<head>
    <meta charset="UTF-8">
    <title>Editar Grupo de Reflexão</title>
...
<body class="container mt-4">
    <h2>Editar Registro - Grupo de Reflexão</h2>
    <form method="post" action="processar_reflexao.php">
        ...
        <div class="mb-3">
            ...
            <select name="tipo" id="tipo" class="form-select">
                <option value="Reflexão" <?= $dado['tipo'] == 'Reflexão' ? 'selected' : '' ?>>Reflexão</option>
                <option value="Palavra do Padre" <?= $dado['tipo'] == 'Palavra do Padre' ? 'selected' : '' ?>>Palavra do Padre</option>
            </select>
        </div>
