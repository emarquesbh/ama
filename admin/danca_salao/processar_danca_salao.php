<?php
// processar_danca_salao.php
// Processa inserção ou atualização de aula de Dança de Salão
...
// Pasta para upload
$pastaUpload = "../uploads/danca_salao/";
...
if (isset($_GET['id'])) {
    ...
    if ($imagem) {
        $stmt = $mysqli->prepare("UPDATE danca_salao SET imagem=?, titulo=?, horarios=?, dia=?, valor=?, turma=?, descricao=?, atualizado_por=?, atualizado_em=? WHERE id=?");
        $stmt->bind_param("sssssdsssi", $imagem, $titulo, $horarios, $dia, $valor, $turma, $descricao, $atualizado_por, $atualizado_em, $id);
    } else {
        $stmt = $mysqli->prepare("UPDATE danca_salao SET titulo=?, horarios=?, dia=?, valor=?, turma=?, descricao=?, atualizado_por=?, atualizado_em=? WHERE id=?");
    ...
    // Novo cadastro
    $stmt = $mysqli->prepare("INSERT INTO danca_salao (imagem, titulo, horarios, dia, valor, turma, descricao, atualizado_por, atualizado_em) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssdsss", $imagem, $titulo, $horarios, $dia, $valor, $turma, $descricao, $atualizado_por, $atualizado_em);
    $stmt->execute();
    $stmt->close();
}

// Redirecionar para listar_danca_salao.php
